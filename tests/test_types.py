...
class TestPackage(TestCase):
    def test_name(self):
        ...
        p = Package(None, path, None, None, None, None)
        assert(p.name() == 'bar')
    ...
    def test_user_flag(self):
        path = os.path.join('foo', 'bar')
        p = Package(None, path, None, None, ['foo'], None)
        assert(p.cflags() == 'foo -Ifoo/bar')

    def test_default_cflag(self):
        path = os.path.join('foo', 'bar')
        p = Package(None, path, None, None, [], [])
        assert(p.cflags() == '-I{}'.format(path))

    def test_default_and_user_cflag(self):
        path = os.path.join('foo', 'bar')
        p = Package(None, path, None, None, ['foo'], None)
        # note that user flags come before the default flags
        assert(p.cflags() == 'foo -I{}'.format(path))

    def test_members(self):
        path    = os.path.join('foo', 'bar')
        members = ['a', 'b']
        p = Package(None, path, members, None, None, None)
        assert(p.members() == members)

class TestGroup(TestCase):
    def test_name(self):
        path = os.path.join('foo', 'bar')
        g = Group(None, path, None, None, None, None)
        assert(g.name() == 'bar')
    ...
    def test_user_flags(self):
        g = Group(None, 'gr1', [], [], ['5'], None)
        assert(g.cflags() == ['5'])

    def test_default_cflags(self):
        path = os.path.join('gr1', 'gr1pkg1')
        pkg1 = Package(None, path, [], [], [], [])

        resolver = dict_resolver({ 'gr1pkg1': pkg1 })

        path = os.path.join('gr1')
        g = Group(resolver, path, frozenset(['gr1pkg1']), None, [], [])
        assert(g.cflags() == ['-Igr1/gr1pkg1'])
    ...
    def test_default_and_user_cflags(self):
        ...
        g = Group(resolver,
                  ...
                  None)
        # note that user flags come before the default flags
        assert(g.cflags() == ['foo', '-Igr1/gr1pkg1'])

    def test_default_ldflags(self):
        path = os.path.join('gr1')
        g = Group(None, path, frozenset(), None, [], [])
        assert(g.ldflags() == ['-Lout/libs', '-lgr1'])
    ...
    def test_default_and_user_ldflags(self):
        ...
        g = Group(resolver,
                  ...
                  ['foo'])
        # note that user flags come before the default flags
        assert(g.ldflags() == ['foo', '-Lout/libs', '-lgr1'])
    ...
    def test_component_with_driver(self):
        ...
        path = os.path.join('gr1')
        g = Group(resolver, path, frozenset(['gr1pkg1']), [], [], [])
        assert(g.components() == ({
            'type':   'object',
            'input':  'gr1/gr1pkg1/gr1pkg1_c1.cpp',
            'cflags': ' -Igr1/gr1pkg1',
...
class TestApplication(TestCase):
    def test_name(self):
        path = os.path.join('foo', 'bar')
        a = Application(None, path, None, None, None, None)
        assert(a.name() == 'bar')
    ...
    def test_user_flags(self):
        a = Application(None, 'gr1', [], [], ['5'], [])
        assert(a.cflags() == ['5'])

    def test_no_dependency_components(self):
        path = os.path.join('foo', 'bar')
        a = Application(None, path, ['a', 'b'], [], [], [])
        assert(a.components() == ({
    ...
    def test_dependency_components(self):
        ...
        a = Application(resolver, 'baz', ['a'], ['gr1'], [], [])
        assert(a.components() == ({
            'input':   'baz/a.cpp',
            'cflags':  '',
            'ldflags': ' -Lout/libs -lgr1',
